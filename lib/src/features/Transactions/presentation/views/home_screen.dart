...
final valueSub = Provider.autoDispose((ref) =>
    (ref.read(totalTransactions('expense')).value ?? 0.0) /
    (ref.watch(totalTransactions('income')).value ?? 0.0));
...
class _TabBodyState extends ConsumerState<TabBody> {
  ...
  Widget build(BuildContext context) {
    ...
    final value = ref.watch(valueSub) * context.getWidth();
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          Money.format(
                            value:
                                (income.value ?? 0.0) - (expense.value ?? 0.0),
            ...
            child: Consumer(
              builder: (BuildContext context, WidgetRef ref, Widget? child) {
                final double totalAmount =
                    ref.watch(totalTransactions(widget.transType)).value ?? 0.0;
